...
const EmergeMaterial = shaderMaterial(
  ...
  /*glsl*/ `
    ...
    float aurora(vec2 uv, float time) {
      float noise = cnoise(vec2(uv.x * 3.0 + time, uv.y * 2.0 - time));
      noise += 0.5 * cnoise(vec2(uv.x * 6.0 - time, uv.y * 4.0 + time));
      return noise;
    }

    vec3 auroraColor(float t) {
      return mix(
        mix(vec3(0.1, 0.5, 0.8), vec3(0.0, 0.8, 0.4), t),
        vec3(0.9, 0.2, 0.5),
        t * t
      );
    }
    ...
    void main() {
      ...
      if(uType==0.){
        ...
        float auroraEffect = aurora(vUv, time);
        vec3 color = auroraColor(auroraEffect);

        float blendFactor = smoothstep(0.2, 0.8, uProgress);
